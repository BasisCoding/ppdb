...
<script>
    ...
    $(document).ready(function() {
        ...
        table = $('#table-pendaftar').DataTable({
            ...
            // "scrollY": "300px",
            "ajax": {
                "url": "<?= base_url('data-pendaftar/store')?>",
                "type": "POST"
            },
    ...
    // BTN Action Modal
        ...
        $('#table-pendaftar').on('click', '.btn-update', function(event) {
            event.preventDefault();
		...
		$('#table-pendaftar').on('click', '.btn-view', function(event) {
            event.preventDefault();
        ...
        $('#table-pendaftar').on('click', '.btn-delete', function(event) {
            event.preventDefault();
		...
		$('#table-pendaftar').on('click', '.btn-validasi', function(event) {
            event.preventDefault();
            var id = $(this).attr('data-id');
            notification('warning', 'Apakah anda yakin ingin validasi registrasi data ini ? <br>'+
        ...
        function send(formData, nameAction) {
            $.ajax({
                url: '<?= base_url("data-pendaftar/") ?>'+nameAction+'',
                type: 'POST',
                dataType: 'JSON',
                data: formData,
                processData: false,
		...
		function detail(id) {
            $.ajax({
                url: '<?= base_url('data-pendaftar/get/') ?>'+id,
                type: 'GET',
                dataType: 'JSON',
                success:function (response) {
