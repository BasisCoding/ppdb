...
class DataPendaftar extends MY_Controller {
	...
	public function index()
	{
		...
		';
		$data['js'] = 'data-pendaftar';
		$data['pages'] = 'ppdb/data-pendaftar';
	...
	{
		$list = $this->PPDBModel->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		foreach ($list as $ls) {
			...
			if ($ls->status_registrasi == 0) {
				$status_registrasi = '<span class="badge rounded-pill badge-danger">Belum Selesai</span>';
			}else if ($ls->status_registrasi == 1) {
				$status_registrasi = '<span class="badge rounded-pill badge-success">Selesai</span>';
			}else{
				$status_registrasi = '';
			}
			...
			<a class="btn btn-sm btn-primary btn-bitbucket btn-validasi" data-id="'.$ls->id.'">
		...
		$output = array(
			"draw"				=> $_POST['draw'],
			"recordsTotal" 		=> $this->PPDBModel->count_all(),
			"recordsFiltered" 	=> $this->PPDBModel->count_filtered(),
			"data" 				=> $data
		);
	...
	public function create()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$act = $this->PPDBModel->create($users, $siswa);
			...
			if ($act) {
				...
				$response = array(
					'type' => 'error',
					'title' => 'Gagal !',
					'message' => 'Data Pendaftar gagal ditambahkan, silahkan coba kembali dalam beberapa menit !'
				);
			}
	...
	{
		$get = $this->PPDBModel->get(str_replace("'", "", htmlspecialchars($id, ENT_QUOTES)));
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$act = $this->PPDBModel->update($data, $id, $users, $user_id);
			// echo $this->db->last_query($act);
			if ($act) {
				// $this->verified_code($data['email']);
				$response = array(
					'type' => 'success',
					'title' => 'Berhasil !',
					'message' => 'Data Pendaftar berhasil diubah !'
				);
			} else {
				$response = array(
					'type' => 'error',
					'title' => 'Gagal !',
					'message' => 'Data Pendaftar gagal diubah, silahkan coba kembali dalam beberapa menit !'
				);
			}
	...
	{
		$user_id = str_replace("'", "", htmlspecialchars($this->input->post('user_id'), ENT_QUOTES));
		$act = $this->PPDBModel->delete($user_id);
		if ($act) {
			// $this->verified_code($data['email']);
			$response = array(
				'type' => 'success',
			...
			$response = array(
				'type' => 'error',
				'title' => 'Gagal !',
				'message' => 'Data Pendaftar gagal dihapus, silahkan coba kembali dalam beberapa menit !'
			);
		}
