...
class PPDBModel extends CI_Model {
	...
	var $column_order = array(null, 'c.nama_lengkap', 'c.nik', 'c.jenis_kelamin', 'u.username', 'u.status'); 
    var $column_search = array('c.nama_lengkap', 'c.nik', 'c.jenis_kelamin', 'u.username', 'u.status'); //field yang diizin untuk pencarian 
    var $order = array('nama_lengkap' => 'asc'); // default order 
    var $table = 'calon_siswa';

// Datatable
    private function _get_datatables_query()
    {
        $this->db->select('c.*, u.username, u.email, u.status, u.foto, j.nama_jurusan as jurusan');
    	$this->db->from($this->table.' as c');
        $this->db->join('users as u', 'u.id = c.user_id', 'left');
		$this->db->join('jurusan as j', 'j.id = c.jurusan_id', 'left');
    	$i = 0;
...
// Datatable	
    ...
    function get($id)
    {
        $this->db->select('u.username, u.email, u.foto, u.status, c.*');
        $this->db->from($this->table.' as c');
        $this->db->join('users as u', 'u.id = c.user_id', 'left');
